...
public class DocOnTheBarItem extends OnTheBarItem {
    ...
    protected void setdate(Calendar sdate, Calendar edate) {
        ...
        if (date[2].length()>2) D = Integer.parseInt(date[2].substring(0,2));
        else D = Integer.parseInt(date[2]);
    ...
    protected void drawItem(float left, float top, float right, float bottom, Canvas canvas) {
        ...
        switch (docType){
            case DOCUMENT :
                canvas.drawRect(left + margin, top + margin, left + interval, bottom-margin, whitePaint);
                canvas.drawRect(left + margin, top + margin, left + interval, bottom-margin, paint);
                break;
            case DECISION :
                canvas.drawRect(left + interval, top + margin, left + interval * 2, bottom-margin, whitePaint);
                canvas.drawRect(left + interval, top + margin, left + interval * 2, bottom-margin, paint);
                break;
            case CHATROOM :
                canvas.drawRect(left + interval * 2, top + margin, right - margin, bottom-margin, whitePaint);
